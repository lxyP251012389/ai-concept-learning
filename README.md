...
- **作用**：给定任意一个学习概念，按固定流程产出八板块学习资料：学习目标 → 核心问题 → 通俗解释（含类比与类比的局限） → 核心机制（含可视化） → 应用场景（含 Mermaid 时序图） → 概念辨析（易混淆对比表 + 常见误解清单 + 使用边界三件套） → 自测问题（3 单选 + 2 简答，每题附「正确答案 + 答错反馈 + 回到正文」三段） → 参考来源（仅官方）
- **质量约束**：参考来源必须是真实可访问的链接（生成时逐一验证 HTTP 200，标注访问日期），禁止编造来源、禁止博客或第三方教程；仅允许官方文档 / arXiv 论文 / 厂商一手技术文档；交付前执行自检清单；HTML 自包含、离线可打开；配色收敛到蓝/橙/灰三色系
...
- 2026-09-10（Skill 第二轮迭代 + 5 份资料全套重写）：按本人对 Skill 的 8 条修订要求整体重写：① 板块 3 改为"通俗解释"（第三人称，去掉第一人称口吻）；② 板块 3 增加"类比 + 类比局限"为强制要素；③ 来源收紧为仅官方（Anthropic 官方工程博客、AWS/Microsoft/NVIDIA 官方文档、arXiv 论文，禁止第三方博客）；④ 配色严格收敛到蓝（#2563eb）+橙（#b45309）+灰（#475569）三色系，删除紫/青/绿/红/粉等所有语义色；⑤ 自测题改为"3 单选 + 2 简答"结构，每个答案含「正确答案 + 答错反馈 + 回到正文」三段；⑥ 概念辨析升级为"易混淆对比表 + 常见误解清单 + 使用边界"三件套；⑦ 学习目标改用"说/区分/解释/举出/判断"等可检验动词，删除"了解/认识"等模糊词；⑧ 解释不再以第一人称表述，强调"本资料代表概念本身"。5 份学习资料（agent / llm-context / skill / rag / concept-relationship）全部按新 Skill 同步重写，形成"Skill 规则 → 资料落地"完整闭环。
